...
public class ExternalChainingHashMap<K, V> {
    ...
    private V put(ExternalChainingMapEntry<K, V> entry) {
    ...
    public V remove(K key) {
        ...
        ExternalChainingMapEntry<K, V> curr = table[index];
        ExternalChainingMapEntry<K, V> dummy = new ExternalChainingMapEntry<>(key, null);

        table[index] = remove(curr, key, dummy);

        return dummy.getValue();
    }

    private ExternalChainingMapEntry<K, V> remove(ExternalChainingMapEntry<K, V> curr, K key, ExternalChainingMapEntry<K, V> dummy) {
        if (curr == null) {
        ...
        if (curr.getKey().equals(key)) {
            ...
            curr.setNext(remove(curr.getNext(), key, dummy));
            return curr;
        }
    }
        ...
        for (ExternalChainingMapEntry<K, V> entry : table) {
            if (entry != null) {
                while (entry != null) {
                    ...
                    // Recalculate index
                    int index = getIndex(entry.getKey(), length);
                    entry.setNext(tempTable[index]);
